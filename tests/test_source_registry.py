...
def test_default_registry_contains_api_and_public_sources(
    ...
    assert registry.create("NGO").name == "ngo"
...
def test_default_registry_exposes_access_policy_metadata() -> None:
    ...
    aplikuj = registry.describe("aplikuj")
    ...
    ngo = registry.describe("ngo")
    assert ngo.access_mode == "public_html"
    assert ngo.experimental is True
    assert ngo.required_env == ()
