...
async def test_benchmark_collector_round_robins_until_target(tmp_path: Path) -> None:
    ...
    assert result.jobs_after == 8
    assert result.rounds == 2
    assert result.sources["a"].runs == 2
    assert result.sources["a"].jobs_inserted == 4
    assert result.sources["b"].runs == 2
    assert result.sources["b"].jobs_inserted == 4


async def test_benchmark_collector_finishes_round_before_target_stop(tmp_path: Path) -> None:
    store = SQLiteStore(tmp_path / "fair-round.sqlite3")
    registry = SourceRegistry()
    registry.register("a", lambda: FakeSource("a"))
    registry.register("b", lambda: FakeSource("b"))
    ...
    assert result.jobs_after == 4
    assert result.sources["a"].runs == 1
