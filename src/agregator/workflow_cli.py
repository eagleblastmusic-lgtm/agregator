...
app = typer.Typer(
    help=(
        "Faro runner. Use 'collect' to gather job offers only and 'enrich' to find "
        "official websites/B2B contacts from an existing database."
    )
)
...
@app.callback()
def main() -> None:
    """Run Faro collection and contact-discovery workflows."""
...
def _pipeline() -> EmployerDiscoveryPipeline:
    return EmployerDiscoveryPipeline(
...
def collect(
    sources: str = typer.Option(
        ...
        "--sources",
        help=(
            "Lista źródeł oddzielona przecinkami albo 'all'. Adaptery wymagające brakujących "
            "poświadczeń są pomijane i raportowane jako skipped."
        ),
    ),
    ...
    fail_fast: bool = typer.Option(
        False,
        "--fail-fast",
        help="Przerwij zbieranie po pierwszym błędzie źródła.",
    ),
...
def enrich(
    ...
    enrichment_limit: int = typer.Option(
        ...
        min=1,
        max=500,
        help="Maksymalna liczba firm analizowanych w tym przebiegu.",
    ),
    ...
    strict: bool = typer.Option(
        False,
        "--strict",
        help="Zwróć kod 2, jeżeli enrichment którejkolwiek firmy zakończy się błędem.",
    ),
) -> None:
    result = asyncio.run(
        run_enrichment_workflow(
    ...
    typer.echo(json.dumps(result.to_dict(), ensure_ascii=False, indent=2))
    if strict and int(result.enrichment.get("failed", 0)):
        raise typer.Exit(code=2)
...
def run(
    ...
    db: str = typer.Option("agregator.sqlite3", "--db"),
    output: str = typer.Option("export/faro_firmy_kontakt.xlsx", "--output"),
    pages_per_source: int = typer.Option(1, "--pages-per-source", min=1, max=1000),
    enrichment_limit: int = typer.Option(
        100,
        "--enrichment-limit",
        min=1,
