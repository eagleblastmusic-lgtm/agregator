...
_DATE = re.compile(
    r"\b(?:oferta\s+opublikowana|posted)\s*:?\s*(\d{1,2})\s+([a-ząćęłńóśźż]+)\s+(\d{4})\b",
    re.IGNORECASE,
)
_LOCATION = re.compile(
    r"(?:^|\n)(?:Lokalizacja|Location)\s*\n\s*([^\n]+)",
    re.IGNORECASE,
)
_MONTHS = {
    "stycznia": "01",
...
class ManpowerPublicSource:
        ...
        try:
            ...
            for index, url in enumerate(links):
                ...
                try:
                    ...
                    continue
                job = parse_manpower_detail(url, detail_html)
                if job is not None:
                    jobs.append(job)
        finally:
            if owns_client:
    ...
    def _parse_page(cursor: str | None) -> int:
        ...
        try:
            ...
            raise ValueError(f"invalid Manpower page cursor: {cursor}") from exc
        if page < 1:
            raise ValueError("Manpower page cursor must be positive")
        return page
    ...
    async def _assert_allowed(self, client: httpx.AsyncClient, url: str) -> None:
        ...
        if not self._robots.can_fetch(self.user_agent, url):
            raise PermissionError(f"robots.txt disallows Manpower URL: {url}")
...
def parse_manpower_detail(url: str, html: str) -> JobPosting | None:
